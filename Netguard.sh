...
DEBUG_MODE=false
DEFAULT_EMAIL=""
MAX_MACS=0  # 0 = unlimited, process all MACs
MIN_PACKETS=5  # Skip MACs with fewer than this many packets
...
#-------------------------------------------------------------------------------
# Funciones de Alertas - Carga de Configuración
#-------------------------------------------------------------------------------
...
send_instant_email() {
    ...
    [ "$1" != "CRITICAL" ] && return  
    command -v msmtp &> /dev/null || return
    ...
    local subject="🚨 NetGuardian Alert #$alert_id"
...
NetGuardian Security System"
    
    {
        ...
        echo "To: $DEFAULT_EMAIL"
        echo ""
        echo "$body"
    } | msmtp "$DEFAULT_EMAIL" 2>/dev/null & 
}
...
║                All-in-One Edition - V3.0.0 (Optimized)               ║
╚═══════════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}"
    echo ""
...
${CYAN}OPCIONES:${NC}
    ...
    -e <email>         Email para notificaciones (requerido si se usa email)
    -o <directory>     Directorio de salida (default: netguardian_TIMESTAMP)
    -m <count>         Limitar análisis a los N MACs más activos (default: todos)
    -p <count>         Umbral mínimo de paquetes por MAC (default: 5)
...
${CYAN}EJEMPLOS:${NC}
    ...
    # Análisis limitado a los 20 MACs más activos (redes grandes)
    sudo $0 -i eth0 -m 20

    # Análisis con umbral personalizado (solo MACs con 10+ paquetes)
    sudo $0 -i eth0 -p 10

    # Análisis con notificación por email
    sudo $0 -i eth0 -e usuario@ejemplo.com
...
validate_duration() {
    ...
    if ! [[ "$duration" =~ ^[0-9]+$ ]]; then
        log_error "Duración inválida: '$duration'"
        echo ""
        echo "La duración debe ser un número entero positivo (en segundos)"
        echo "Ejemplo: -d 60"
        echo ""
        exit 1
    fi

    # Validar rango razonable (1 segundo a 1 hora)
    if [ "$duration" -lt 1 ] || [ "$duration" -gt 3600 ]; then
        log_error "Duración fuera de rango: $duration segundos"
        echo ""
        echo "La duración debe estar entre 1 y 3600 segundos (1 hora)"
        echo ""
        exit 1
    fi
}
...
validate_interface() {
    ...
    # Sanitizar: solo permitir caracteres alfanuméricos, guiones y puntos
    if ! [[ "$iface" =~ ^[a-zA-Z0-9._-]+$ ]]; then
        log_error "Nombre de interfaz inválido: '$iface'"
        echo ""
        echo "El nombre de interfaz solo puede contener letras, números, puntos, guiones y guiones bajos"
        echo ""
        exit 1
    fi

    # Verificar que la interfaz existe
    if ! ip link show "$iface" &> /dev/null && ! tshark -D 2>/dev/null | grep -q "$iface"; then
        log_error "Interfaz no encontrada: '$iface'"
        echo ""
        echo "Usa: $0 -l para listar interfaces disponibles"
        echo ""
        exit 1
    fi

    log_verbose "Interfaz validada: $iface"
}
...
analyze_fingerprinting() {
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    log_success "Analizando huellas digitales de dispositivos (OPTIMIZED)"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo ""

    log "Extrayendo todos los datos en una sola pasada..."

    # SINGLE TSHARK PASS - Extract all data at once (10-20x faster!)
    tshark -r "$PCAP_FILE" -T fields \
        ...
        -E separator='|' \
        -E occurrence=f \
        2>/dev/null > "$TEMP_DIR/fingerprint/all_data.txt"

    if [ ! -s "$TEMP_DIR/fingerprint/all_data.txt" ]; then
        log_error "No se pudieron extraer datos del PCAP"
        echo "[]" > "$FINGERPRINT_FILE"
        return 1
    fi

    local total_packets=$(wc -l < "$TEMP_DIR/fingerprint/all_data.txt")
    log_verbose "Total de paquetes extraídos: $total_packets"

    log "Procesando estadísticas por dispositivo..."

    # Process all data with awk - MUCH faster than multiple tshark calls
    awk -F'|' -v min_packets="$MIN_PACKETS" '
    BEGIN {
        OFS="|"
    }
...

    {
        ...
        # Count packets per MAC
        packets[mac]++

        # Sum bytes per MAC
        bytes[mac] += frame_len

        # Count protocols per MAC
        if (protocol != "") {
            proto_key = mac "|" protocol
            protocols[proto_key]++
            # Track unique protocols for device type estimation
            if (!(mac "|plist|" protocol in seen_proto)) {
                proto_list[mac] = proto_list[mac] " " protocol
                seen_proto[mac "|plist|" protocol] = 1
            }
        }

        # Collect unique IPv4 addresses
        if (ipv4 != "" && !(mac "|ipv4|" ipv4 in seen_ipv4)) {
            if (ipv4_addrs[mac] != "") {
                ipv4_addrs[mac] = ipv4_addrs[mac] "," ipv4
            } else {
                ipv4_addrs[mac] = ipv4
            }
            seen_ipv4[mac "|ipv4|" ipv4] = 1
        }

        # Collect unique IPv6 addresses (limit to 3 per MAC)
        if (ipv6 != "" && !(mac "|ipv6|" ipv6 in seen_ipv6)) {
            ipv6_count[mac]++
            if (ipv6_count[mac] <= 3) {
                if (ipv6_addrs[mac] != "") {
                    ipv6_addrs[mac] = ipv6_addrs[mac] "," ipv6
                } else {
                    ipv6_addrs[mac] = ipv6
                }
            }
            seen_ipv6[mac "|ipv6|" ipv6] = 1
        }

        # Track first and last seen timestamps
        if (!(mac in first_seen) || timestamp < first_seen[mac]) {
            first_seen[mac] = timestamp
            first_seen_time[mac] = frame_time
        }
        if (!(mac in last_seen) || timestamp > last_seen[mac]) {
            last_seen[mac] = timestamp
            last_seen_time[mac] = frame_time
        }
    }

    END {
        # Output summary for each MAC that meets minimum packet threshold
        for (mac in packets) {
            if (packets[mac] >= min_packets) {
                # Output format: mac|packets|bytes|ipv4_list|ipv6_list|first_seen|last_seen|protocols
                print mac, packets[mac], bytes[mac], \
                      (ipv4_addrs[mac] ? ipv4_addrs[mac] : ""), \
                      (ipv6_addrs[mac] ? ipv6_addrs[mac] : ""), \
                      first_seen_time[mac], last_seen_time[mac], \
                      proto_list[mac]
            }
        }
    }
    ' "$TEMP_DIR/fingerprint/all_data.txt" | \
    sort -t'|' -k2,2rn > "$TEMP_DIR/fingerprint/mac_summary.txt"

    # Sort by packet count and limit if MAX_MACS is set
    if [ "$MAX_MACS" -gt 0 ]; then
        head -n "$MAX_MACS" "$TEMP_DIR/fingerprint/mac_summary.txt" > "$TEMP_DIR/fingerprint/mac_summary_limited.txt"
        mv "$TEMP_DIR/fingerprint/mac_summary_limited.txt" "$TEMP_DIR/fingerprint/mac_summary.txt"
        log_verbose "Limitado a los $MAX_MACS dispositivos más activos"
    fi

    local mac_count=$(wc -l < "$TEMP_DIR/fingerprint/mac_summary.txt")
    log_success "Encontrados $mac_count dispositivos únicos"

    # Extract top 10 protocols per MAC
    log "Extrayendo protocolos principales..."
    awk -F'|' '
    $1 != "" && $4 != "" {
        mac = $1
        protocol = $4
    ...
    END {
        for (key in protocols) {
            split(key, parts, "|")
            print parts[1] "|" parts[2] "|" protocols[key]
        }
    }
    ' "$TEMP_DIR/fingerprint/all_data.txt" | \
    sort -t'|' -k1,1 -k3,3rn | \
    ...
    ' > "$TEMP_DIR/fingerprint/protocols.txt"
    ...
    # Build JSON from processed data
    while IFS='|' read -r mac packets bytes ipv4_list ipv6_list first_seen last_seen proto_hint; do
        [[ -z "$mac" ]] && continue

        processed=$((processed + 1))

        if [[ "$VERBOSE" == true ]]; then
            printf "\r  Generando JSON: %d/%d" "$processed" "$mac_count"
        fi

        # Get vendor from OUI database
        local vendor=$(get_vendor "$mac")

        # Get top protocols for this MAC and format as JSON
        local protocols=$(awk -F'|' -v m="$mac" '
            $1 == m {
                printf "\"%s\":%d,", $2, $3
            }
        ' "$TEMP_DIR/fingerprint/protocols.txt" | sed 's/,$//')
        ...
        local device_type=$(estimate_device_type "$mac" "$vendor" "$proto_hint" "$packets")

        # Format IP address arrays for JSON
        local ipv4_json=""
        if [[ -n "$ipv4_list" ]]; then
            ipv4_json=$(echo "$ipv4_list" | awk -F',' '{for(i=1;i<=NF;i++) printf "\"%s\",", $i}' | sed 's/,$//')
        fi

        local ipv6_json=""
        if [[ -n "$ipv6_list" ]]; then
            ipv6_json=$(echo "$ipv6_list" | awk -F',' '{for(i=1;i<=NF;i++) printf "\"%s\",", $i}' | sed 's/,$//')
        fi
...
EOF
    ...
    done < "$TEMP_DIR/fingerprint/mac_summary.txt"
...
EOF
    ...
    # Write JSON file with pretty formatting if jq available
...
while getopts "i:d:o:e:m:p:vnDhltV" opt; do
    case $opt in
        i) INTERFACE="$OPTARG" ;;
        d) DURATION="$OPTARG" ;;
        o) OUTPUT_DIR="$OPTARG" ;;
